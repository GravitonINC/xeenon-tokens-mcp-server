...
const quoteSellParamsSchema = z.object({
...
export const quoteSell = async ({
  tokenAmount,
  token,
}: z.infer<typeof quoteSellParamsSchema>) => {
  const sdk = await tokenAddressToMarketLinearWithMetaSdk(token);

  const quote = sdk.quoteSellWithFee(tokenAmount);

  return quote;
};

export const registerQuoteSell = (server: McpServer) => {
  server.registerTool(
    'quoteSell',
    {
      title: 'Get a quote to sell a token',
      description: 'Get a quote to sell a token with a given amount of tokens',
      inputSchema: quoteSellParamsSchema.shape,
    ...
    async (args) => {
      try {
        const quote = await quoteSell(args);
