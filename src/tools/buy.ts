import { z } from 'zod';
import { getToken, tokenAddressToMarketLinearWithMetaSdk } from '../util/token';
...
const quoteBuyParamsSchema = z.object({
  ...
  slippage: z
    .number()
    .describe(
      'The slippage percentage to use to buy the token. I.e. 0.1 means 0.1%'
    )
    .optional()
    .default(1),
});
...
export const buy = async ({
  ...
  const minOutAmount = crediezAmount - (crediezAmount * slippage) / 100;
